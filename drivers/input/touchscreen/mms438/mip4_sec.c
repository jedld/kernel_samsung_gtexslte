...
static void cmd_fw_update(void *device_data)
{
	...
	switch(fw_location){
		case 0:
			...
			break;
		case 1:
			if(mip_fw_update_from_storage(info, true)){
				goto ERROR;
			}
			break;
		default:
...
/* FIXME: firmware V1.11 touch threshold. We need to update the Register*/
...
static void cmd_run_test_cm_abs(void *device_data)
{
	...
	for (i = 0; i < (info->node_x * info->node_y); i++) {
		...
		if (info->image_buf[i] < min)
			min = info->image_buf[i];
		dev_info(&info->client->dev, "data[%d] = %d\n", i, info->image_buf[i]);
	}
...
static void dead_zone_enable(void *device_data)
{
	...
	if ((enable == 0) || (enable == 1)) {
		if (mip_i2c_write(info, wbuf, 3)) {
			...
			dev_dbg(&info->client->dev, "%s - value[%d]\n", __func__, wbuf[2]);
